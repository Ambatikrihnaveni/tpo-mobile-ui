...
const InternshipItem = ({ record, view, i }) => {
    ...
    const handleItemAction = (menuItem) => {
        switch (menuItem.action) {
            ...
            case 'delete':
                showDialog({
                    variant: 'confirm',
                    title: 'Are you sure about remove this program?',
                    content: "You won't be able to recover this program later",
                    onYes: async () => {
                        try {
            ...
            <JumboGridItem xs={12} sm={4} md={4} lg={3}>
                <Card sx={{ boxShadow: '0px 4px 8px rgba(0, 0, 0, 0.4)' }}>
                    ...
                    <CardContent sx={{
                        ...
                        <Typography variant={"h5"} mb={.75} style={{ fontWeight: 'bold', fontSize: '20px', marginTop: '-10px' }}>{(truncateName && truncateName?.charAt(0).toUpperCase() + truncateName.slice(1))}</Typography>
                        <Div sx={{ mb: 1 }}>
                            ...
                            <Stack direction={"row"} alignItems={"center"} sx={{ minWidth: 0 }}>
                                ...
                                >
                                    ...
                                    <Typography variant={"h5"} fontSize={16} lineHeight={1.25} mb={0}
                                        noWrap> <Tooltip title={record?.name}>
                                ...
                                <Item>
                                    <Button onClick={() => handleItemAction({ action: 'delete' })} sx={{ color: '#9d2c44', ml: 5 }} >Remove</Button>
                                </Item>
                            </Stack>
                        </Typography>
                    }
