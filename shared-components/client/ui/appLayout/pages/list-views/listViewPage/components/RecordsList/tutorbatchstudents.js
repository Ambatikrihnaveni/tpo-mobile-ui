...
function TutorBatchStudents({ scrollHeight, record }) {
  ...
  React.useEffect(() => {
    const filtered = record?.students?.filter(data =>
      ...
      action={
        ...
        <Stack direction={"row"} spacing={1} alignItems={"center"}>
          <Dialog open={dialogOpen} onClose={closeDialog} sx={{ Width: { md: '750px' } }} >
            ...
            <DialogContent sx={{ marginTop: '40x', width: '500px', justifyContent: 'center', padding: '15px' }}>
        ...
        <List >
          {
            filteredStudents?.map((students, index) => (
              ...
              >
                ...
                <ListItemIcon sx={{ minWidth: 40, mt: 0 }}>
                  <Checkbox edge="start" checked={selected.indexOf(students) !== -1} tabIndex={-1} onChange={(event) => handleClick(event, students)} />
                </ListItemIcon>

                <ListItemAvatar >
                  <Avatar src={""} alt={students.name} />
                </ListItemAvatar>
                <ListItemText
                      ...
                      <Stack direction={"row"} alignItems={"center"} sx={{ minWidth: 0 }}>
                        ...
                        >
                          <Typography variant={"h5"} fontSize={14} lineHeight={1.25} mb={0}
                            noWrap> {students?.name && students?.name.charAt(0).toUpperCase() + students?.name.slice(1)}  </Typography>
