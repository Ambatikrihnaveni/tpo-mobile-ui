...
const ProgramSidebar = ({ data }) => {

    const [showFullContent, setShowFullContent] = useState(false);
    const { viewer } = useAuth()
    let imageSrc = (data?.programMedia) ? data?.programMedia[0]?.URLs?.small : '';
        ...
        <Grid>
            ...
            {viewer?.role === "Master-Admin" ? (null) : (
                <div style={{ display: 'flex', alignItems: 'center', marginTop: '15px', marginBottom: '10px' }}>
                    ...
                    <div style={{ fontSize: '20px', marginLeft: '10px', fontWeight: 'bold', textTransform: 'capitalize' }}>  {data?.field}
                    </div>
                </div>
            )}
            ...
            <Typography
                sx={{
                    display: '-webkit-box',
                    WebkitLineClamp: showFullContent ? 'unset' : 2,
                    WebkitBoxOrient: 'vertical',
                    overflow: 'hidden',
                    marginTop: "15px",
                    minHeight: 40
                }}>
                <div dangerouslySetInnerHTML={{ __html: showFullContent ? data?.program_content || data?.description : (data?.program_content || data?.description) }} />
            </Typography>
            <Button onClick={toggleContent} variant="text" size="small" color="primary" sx={{ float: 'right', marginTop: '-5px', marginLeft: '10px' }}>
