...
function ProductsTable() {
  ...
  const columns = useMemo(() => [
    ...
    {
      Header: "Course"/* i18next.t("admin.productTable.header.product") */,
      accessor: "title"
    },
    {
  ...
  const handleCreateProduct = async () => {
    ...
    if (data) {
      ...
      //navigate(`/${shopId}/mycourses/${product._id}`);
      navigate(`/${shopId}/products/${product._id}`);
    }
      ...
      { (!isTagSelectorVisible && !isFilterByFileVisible) &&
        <Grid item sm={12}>
          <Button color="primary" variant="contained" onClick={handleCreateProduct}>
            {/* {i18next.t("admin.createProduct") || "Create Course"} */}
            Create Course
          </Button>
        </Grid>
      }
