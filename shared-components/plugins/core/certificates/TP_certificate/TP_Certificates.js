...
import { Card, Button, Grid, Typography, IconButton } from '@mui/material'
...
export default function TP_Certificates() {
  ...
  React.useEffect(async () => {
    ...
    const data = await CertificatesServices.getCertificates(shopId)
    if (data) {
  ...
  const onAdd = () => {
    showDialog({
      content: <CreateCertificate hideDialog={hideDialog} setRefresh={setRefresh} />,
      sx: {
        "& .MuiDialog-container": {
          "& .MuiPaper-root": {
            width: "1200px",
  ...
  const deleteRecordMutation = useMutation(CertificatesServices.removeCertificate, {
  ...
  const onDedlete = (data) => {
    const id =  data?._id
    
    showDialog({
      variant: 'confirm',
      title: 'Are you sure about deleting this template?',
      content: "You won't be able to recover this template later",
      onYes: async () => {
        try {
          await deleteRecordMutation.mutateAsync({shopId,id});
          hideDialog();
  ...
  return (
    <JumboCardQuick
      title={'Certificates'}
      action={<Button variant="contained" sx={{ textTransform: 'none' }} onClick={onAdd}>Add</Button>}
      sx={{ minHeight: '450px' }}
    >
      {isLoading ?
        <Div
                  ...
                  {hoveredIndex === i && (
                    <Div>
                      <Div sx={{ textAlign: 'right' }}>
                        <IconButton variant="contained" color='primary' onClick={() => onDedlete(data)} >
                          <DeleteIcon />
                        </IconButton>
                      </Div>
                      <Div sx={{ textAlign: 'center', marginTop: '70px' }}>
      ...
      {isViewerOpen && (
        <ImageViewer
          src={certificates.map((data) => data.templateImage)}
