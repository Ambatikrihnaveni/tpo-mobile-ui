...
export default function QuizItem() {
  ...
  const [quizAttempt] = useMutation(studentQuiz, { ignoreResults: true });
  ...
  const showRecordDetail = React.useCallback(() => {
    showDialog({
      fullScreen: true,
      content: <QuizSubmit onClose={hideDialog} lessonId={lessonId} />
    })
  }, [showDialog]);
  ...
  const calculateProgressPercentage = () => {
    ...
    return ((currentQuestionIndex + 1) / quizzes.length) * 100;
  };
      ...
      <Div sx={{ marginTop: '3%', marginLeft: '5%' }}>
        ...
        <Div sx={{ marginTop: '3%', marginLeft: '13%' }}>
          <Typography sx={questionStyles}>
            {quizzes[currentQuestionIndex]?.quiz_question}
          </Typography>
        </Div>
        <Div sx={{ marginTop: '2%', display: 'flex', alignItems: 'center', margin: '5%', marginLeft: '14%' }}>
          <Grid container spacing={isMobile ? 2 : 0}>
            <Grid item xs={12} md={6}>
              <TextField
                value={quizzes[currentQuestionIndex]?.quiz_options?.optionA}
                sx={textFieldStyles}
                size="medium"
                variant="outlined"
                InputProps={{
            ...
            <Grid item xs={12} md={6}>
              <TextField
                value={quizzes[currentQuestionIndex]?.quiz_options?.optionB}
                sx={textFieldStyles}
                size="medium"
                variant="outlined"
                InputProps={{
        ...
        <Div sx={{ marginTop: '2%', display: 'flex', alignItems: 'center', margin: '5%', marginLeft: '14%' }}>
          <Grid container spacing={isMobile ? 2 : 0}>
            <Grid item xs={12} md={6}>
              <TextField
                value={quizzes[currentQuestionIndex]?.quiz_options?.optionC}
                sx={textFieldStyles}
                size="medium"
                variant="outlined"
                InputProps={{
            ...
            <Grid item xs={12} md={6}>
              <TextField
                value={quizzes[currentQuestionIndex]?.quiz_options?.optionD}
                sx={textFieldStyles}
                size="medium"
                variant="outlined"
                InputProps={{
