...
const Friends = () => {
    ...
    return (
        <JumboCardQuick style={{ minHeight: '150px' }}
            title={
                role === 'Tutor' || role === 'Student' ? 'Skills' : role === 'College-Admin' ? 'Management Details' : 'Tutors'
            }
            wrapperSx={{ pt: 0 }}
        >
            {role === 'Tutor' ? <TutorSkills /> : role === 'Student' ? <StudentSkills/> : role === 'College-Admin' ? <ManagementDetails/> :
                            ...
                            <Item key={index}>
                                <StyledBadge overlap={"circular"} variant={"dot"} color={item.color}>
                                    <StyledAvatar
                                        ...
                                        variant={"rounded"}
                                    >
                                        {item.name.charAt(0).toUpperCase()} {/* Display the first letter of item.name */}
                                    </StyledAvatar>
