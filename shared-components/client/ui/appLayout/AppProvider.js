...
function init(initialValue) {
    return {
        ...
        rebuildRoutes: true,
        chatWidgetVisibility:initialValue.chatWidgetVisibility
    }
}
...
function appReducer(state, action) {
    switch(action.type) {
        ...
        case APP_ACTIONS.SET_CHAT_WIDGET_VISIBILITY:
            return {
                ...state,
                chatWidgetVisibility: action.payload,
            };
...
const AppProvider = ({children}) => {
    ...
    const setChatWidgetVisibility = React.useCallback((value) => {
        setApp({type: APP_ACTIONS.SET_CHAT_WIDGET_VISIBILITY, payload: value});
    }, [setApp]);
