...
function ProgramTutors({ scrollHeight, record, tutors }) {

  const [activeCategory, setActiveCategory] = React.useState("all");
  const [filteredTasks, setFilteredTasks] = React.useState(tutors);
  ...
  const [filteredTutors, setFilteredTutors] = React.useState(tutors)
  ...
  React.useEffect(() => {
    if (activeCategory && activeCategory !== "all")
      setFilteredTasks(tutors.filter(task => task.category === activeCategory));
    else
      setFilteredTasks(tutors);
  ...
  const handleSelectAllClick = (event) => {
    ...
    if (event.target.checked) {
      const newSelected = tutors.map((n) => n);
                      ...
                      <Stack direction={"row"} alignItems={"center"} sx={{ minWidth: 0 }} onClick={() => viewProfile(tutor._id)}>
                        ...
                        >
                          <Typography variant={"h5"} fontSize={14} lineHeight={1.25} mb={0}
                            noWrap> {tutor.name && tutor.name.charAt(0).toUpperCase() + tutor.name.slice(1)}                                        </Typography>
