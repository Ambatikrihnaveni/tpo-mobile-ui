...
import {
  Box, Typography, Grid, MenuItem, Select, FormControl, OutlinedInput, InputLabel, Autocomplete, TextField,
...
export default function PlacementModules(params) {
  ...
  const lessonValues = (Ids, lessons) => {
    let lessonsData = []
    if (Ids.length > 0) {
      for (let i = 0; i < Ids.length; i++) {
        for (let x = 0; x < lessons.length; x++) {
          if (Ids[i] == lessons[x]._id) {
            lessonsData.push(lessons[x].name)
          }
        }
      }
    }
  ...
  return (
    <Div>
      <Box sx={{ marginTop: recordType ? "" : '-80px', height: '480px' }}>
        <JumboScrollbar
          autoHide
          ...
          autoHeightMin={500}
        >
          <Typography variant='h1' sx={{ m: 2, textAlign: 'left', fontWeight: 'bold', color: "black", textTransform: 'none' }}>Add Modules</Typography>
