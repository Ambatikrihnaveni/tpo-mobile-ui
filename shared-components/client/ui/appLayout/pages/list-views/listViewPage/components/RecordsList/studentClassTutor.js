...
function StudentClassTutor({ scrollHeight,record}) {
  const [activeCategory, setActiveCategory] = React.useState("all");
  const [filteredTasks, setFilteredTasks] = React.useState(record?.tutors);
  ...
  const [filteredTutors,setFilteredTutors]= React.useState(record?.tutors)
...
React.useEffect(() => {
   ;
  const filtered = record?.tutors?.filter(tutor =>
    tutor.name.toLowerCase().includes(searchKeywords.toLowerCase())
  );
...
const onSend = async () => {
  ;
  const shopId= (viewer?.shopId.length > 0) ? (viewer?.shopId[0]._id):''
  let accountIds = []
...
const onEditorChange=()=>{
  ...
  React.useEffect(() => {
    if (activeCategory && activeCategory !== "all")
      setFilteredTasks(record?.tutors?.filter(task => task.category === activeCategory));
    else
      setFilteredTasks(record?.tutors);
  ...
  const handleSelectAllClick = (event) => {
    ...
    if (event.target.checked) {
      const newSelected = record?.tutors?.map((n) => n);
  ...
  const handleEmojiClick = (event, emojiObject) => {
     ;
   // const newOverview = overview + emojiObject.emoji;
   const newOverview = `${overview}${emojiObject.emoji}`;
        ...
        <List >
          {
            filteredTutors?.map((tutor, index) => (
              ...
              >
                ...
                <ListItemIcon sx={{ minWidth: 40, mt: 0 }}>
                  <Checkbox edge="start" checked={selected.indexOf(tutor) !== -1} tabIndex={-1} onChange={(event) => handleClick(event, tutor)} />
                </ListItemIcon>
                ...
                <ListItemText
                  ...
                  primary={
                    <Typography variant={"body1"} component={"div"}>
                      <Stack direction={"row"} alignItems={"center"} sx={{ minWidth: 0 }} onClick={()=>viewProfile(tutor._id)}>
                        <Item
                          sx={{
...
export default StudentClassTutor