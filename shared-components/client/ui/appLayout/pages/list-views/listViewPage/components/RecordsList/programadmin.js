...
function ProgramAdmin({ scrollHeight, record }) {
  const [activeCategory, setActiveCategory] = React.useState("all");
  const [filteredTasks, setFilteredTasks] = React.useState(record?.admins);
  ...
  const [filteredAdmins, setFilteredAdmins] = React.useState(record?.admins);
  ...
  React.useEffect(() => {
    const filtered = record?.admins?.filter(admin =>
      admin?.name?.toLowerCase().includes(searchKeywords.toLowerCase())
    );
  ...
  React.useEffect(() => {
    if (activeCategory && activeCategory !== "all")
      setFilteredTasks(record?.admins?.filter(task => task.category === activeCategory));
    else
      setFilteredTasks(record?.admins);
  ...
  const handleSelectAllClick = (event) => {
    ...
    if (event.target.checked) {
      const newSelected = record?.admins.map((n) => n);
        ...
        <List >
          {
            filteredAdmins?.map((admin, index) => (
              ...
              >
                ...
                <ListItemIcon sx={{ minWidth: 40, mt: 0 }}>
                  <Checkbox edge="start" checked={selected.indexOf(admin) !== -1} tabIndex={-1} onChange={(event) => handleClick(event, admin)} />
                </ListItemIcon>

                <ListItemAvatar >
                  <Avatar src={""} alt={admin.name} />
                </ListItemAvatar>
                <ListItemText
                  // onClick={showtutorDetail}
                  primary={
                    <Typography variant={"body1"} component={"div"}>
                      <Stack direction={"row"} alignItems={"center"} sx={{ minWidth: 0 }} onClick={() => viewProfile(admin._id)}>
                        <Item
                          sx={{
                        ...
                        >
                          <Typography variant={"h5"} fontSize={14} lineHeight={1.25} mb={0}
                            noWrap> {admin.name && admin.name.charAt(0).toUpperCase() + admin.name.slice(1)}                                        </Typography>
