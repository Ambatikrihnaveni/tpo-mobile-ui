...
import deleteCompetitivePrepMutation from '../../../../../graphql/services/competitive_Prep/mutations/deleteCompetitivePrep';
import SettingsIcon from '@mui/icons-material/Settings';
import VisibilityIcon from '@mui/icons-material/Visibility';
import { useJumboDialog } from "../../../../../@jumbo/components/JumboDialog/hooks/useJumboDialog"
import CompetitivePreview from '../../../../../../../plugins/core/library/client/competitivePreview/competitivePreview';
...
const EntranceExamItem = ({ record, view, i }) => {
    const navigate = useNavigate();
    const { showDialog, hideDialog } = useJumboDialog();
    const [deleteCompetitivePrep] = useMutation(deleteCompetitivePrepMutation)
    const { setRecordsListRefresh } = useListViewpage()
    const handleDetailsClick = () => {
        navigate(`/entranceexam/quiz`)
    ...
    const onProgram = () => {
        showDialog({
            fullScreen: true,
            content: <CompetitivePreview handleClose={hideDialog} record={record} />,
            sx: {
                borderRadius: 0
            }
        })
    ...
    const onEdit = React.useCallback(async (menuItem) => {
        ...
        const entranceexamId = record?.id;
        navigate(`/competitivepreep/entranceexam/${entranceexamId}/edit`);
    }, [navigate]);
        ...
        switch (menuItem.action) {
            ...
            case 'delete':
                showDialog({
                    ...
                    onYes: async () => {
                        try {
                            const competitivePrepIdToDelete = [menuItem.data.id];
                            await deleteCompetitivePrep({
                                variables: {
                                    competitivePrepIds: competitivePrepIdToDelete,
                                },
                            });
                ...
                <Card sx={{ boxShadow: '0px 4px 8px rgba(0, 0, 0, 0.4)', width: '250px' }}>
                    ...
                    <CardActions sx={{ p: 0, mx: '-1px' }}>
                        <ButtonGroup size="large" fullWidth variant="outlined">
                            <ActionButton >Learn</ActionButton>
                    ...
                    primary={
                        <Typography variant={"body1"} component={"div"}>
                            <Stack direction={"row"} alignItems={"center"} sx={{ minWidth: 0 }}>
                                ...
                                >
                                    <Typography variant={"body1"} noWrap> {(record?.lessons)} </Typography>
                                </Item>
                                ...
                                >
                                    <Typography variant={"body1"} noWrap> {(record?.quizes)} </Typography>
                                </Item>
                                ...
                                >
                                    <Chip label="Start" style={{ backgroundColor: '#7492a3', color: 'white' }} />
                                </Item>
...
export default EntranceExamItem;
