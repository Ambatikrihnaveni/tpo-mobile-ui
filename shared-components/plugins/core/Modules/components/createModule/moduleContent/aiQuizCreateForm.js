...
export default function AiQuizCreateForm(props) {
  ...
  const { onUpdateQuiz, onCreateQuiz, onDeleteQuiz, onCreteAiService } = useQuiz({ lessonId, Id: productId })
  ...
  const onSubmit = useCallback(async () => {
    ...
    try {
      const data = await onCreteAiService({ lessonId, productId })
...
return (
  <Div sx={{ p: 3 }}>
      ...
      <Grid container>
        <Grid xs={11}>
          <Typography variant={'h5'} fontSize={23} sx={{ fontWeight: 'bold' }}>
            {' '}
            <AutoAwesomeIcon /> AI Quiz Create
          </Typography>
        </Grid>
        <Grid xs={1}>
          <IconButton onClick={handleCancel}>
    ...
    <Div sx={{ textAlign: 'center' }}>
      ...
      <Typography sx={{ display: loading ? 'block' : 'none', mt: 3 }}>{`We are preparing quizzes for ${lesson?.name}`}</Typography>
    </Div>
    <ToastContainer
      position="bottom-right"
      autoClose={3000}
